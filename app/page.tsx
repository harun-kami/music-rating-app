...
export default function Home() {
  ...
  // 1. Recent Digs (日付順に最新を取得)
  const recentDigs = [...reviews]
    .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

  // 2. Top Ranking (スコア順に10枚)
  const topRanking = [...reviews]
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {reviews.length === 0 ? (
          ...
          <div className="space-y-20">
            ...
            {/* --- SECTION 1: RECENT COLLECTION (Original Carousel Style) --- */}
  ...
  {recentDigs.map((rev) => (
    <div key={rev.id} className="flex-none w-[140px] group text-left">
      ...
      <span className="text-lg font-black text-orange-500 italic leading-none">{rev.score.toFixed(1)}</span>
            ...
            {/* --- SECTION 2: TOP 10 GRAILS (The Grid Style from current code) --- */}
            <section>
              <h2 className="text-[10px] font-black border-l-2 border-orange-500 pl-3 uppercase tracking-[0.2em] text-gray-500 mb-8 text-left">The Grails / Top 10</h2>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                {topRanking.map((review, index) => (
