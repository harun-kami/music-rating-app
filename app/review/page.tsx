...
function ReviewContent() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch(`https://itunes.apple.com/search?term=${encodeURIComponent(query)}&entity=album&limit=20&country=JP&lang=ja_jp`);
      const data = await res.json();
          ...
          <>
            ...
            <div className="bg-[#1e1e1e] p-6 md:p-8 rounded-[2rem] md:rounded-3xl mb-12 flex flex-col md:flex-row justify-between items-center border border-gray-800 shadow-xl gap-6 md:gap-8">
              <div className="flex gap-4 md:gap-6 items-center min-w-0 w-full md:w-auto">
                <img src={selectedAlbum.image} className="w-20 h-20 md:w-32 md:h-32 rounded-xl shadow-2xl object-cover border border-white/5 flex-none" alt="" />
              ...
              <button onClick={handleSave} className="w-full md:w-auto px-10 py-4 md:py-5 bg-orange-500 text-black rounded-2xl font-black transition-all active:scale-95 shadow-xl hover:scale-105 text-xs md:text-base">{saveStatus || "SAVE DIG"}</button>
            </div>
                ...
                <div key={i} className="flex flex-col bg-[#1e1e1e]/50 rounded-2xl border border-gray-800 overflow-hidden transition-all">
                  ...
                  <div className={`px-6 sm:px-14 transition-all duration-300 ease-in-out bg-black/20 ${expandedTrack === i ? 'max-h-40 pb-5 pt-2 opacity-100 border-t border-gray-800/50' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                    <div className="space-y-2 pt-3">
                      ...
                      <div className="flex gap-2 text-[8px] md:text-[9px] uppercase tracking-[0.2em]"><span className="text-orange-500/40 font-black italic">Writers</span><span className="text-gray-500 font-bold">Data syncing from archive...</span></div>
                    </div>
                  </div>
                </div>
              ))}
