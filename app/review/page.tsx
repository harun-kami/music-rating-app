              ...
              <div className="flex overflow-x-auto gap-6 pb-12 scrollbar-hide snap-x">
                {results.map((album) => (
                  <div 
                    ...
                    onClick={() => {
                      router.push(`/review?id=${album.collectionId}`);
                  ...
                  >
                    ...
                    {/* アーティスト名からのリンク (e.stopPropagationで親のクリックを阻止) */}
                    <p 
                      onClick={(e) => {
                        e.stopPropagation();
                        router.push(`/artist/${album.artistId}`);
                      }}
                      className="text-gray-600 hover:text-orange-500 text-[8px] font-bold uppercase tracking-[0.2em] transition-colors inline-block"
                    >
                      {album.artistName}
                    </p>
                  </div>
          ...
          <>
            <header className="flex justify-between items-center mb-10">
              <button 
                ...
                className="text-gray-500 hover:text-orange-500 text-xs font-bold uppercase"
              ...
              <div className="flex gap-6 items-center min-w-0">
                ...
                <div className="min-w-0 text-left">
                  ...
                  <div className="flex items-center gap-4">
                    {/* 評価ページのアーティスト名もリンク化 */}
                    <Link href={`/artist/${selectedAlbum.artistId}`}>
                      <p className="text-xs text-gray-500 hover:text-orange-500 font-bold uppercase truncate transition-colors leading-none">
              ...
              {selectedAlbum.tracks.map((track: string, i: number) => (
                <div key={i} className="flex justify-between items-center bg-[#1e1e1e]/50 p-5 rounded-2xl border border-gray-800">
                  <div className="flex items-center gap-4 truncate text-left">
                    <button onClick={() => setFavoriteTrack(i === favoriteTrack ? null : i)} className={`transition-all active:scale-125 ${favoriteTrack === i ? 'text-orange-500' : 'text-gray-700 hover:text-orange-500/50'}`}>
